      ...
      <table className="w-full text-xs">
        <thead className="bg-slate-50 sticky top-0">
          <tr>
            ...
            <th className="text-right p-2 font-medium text-blue-600">
              타사 단가계
            </th>
        ...
        <tbody>
          {origItems.map((orig) => {
            ...
            return (
              ...
              >
                <td
                  className="p-2 font-medium text-slate-800 max-w-[160px] truncate"
                  title={orig.itemName}
                >
                  {orig.itemName}
                  {orig.spec && (
                ...
                {ri ? (
                  <>
                    <td
                      className="p-2 text-blue-700 max-w-[160px] truncate"
                      title={ri.itemName}
                    >
                      {ri.itemName}
                    </td>
                    ...
                    <td className="p-2 text-right">
                      <DevBadge pct={ri.origDeviationPct} />
                    </td>
                  </>
                ) : (
          ...
          {unmatched.map((ri) => (
            <tr
              key={`u-${ri.id}`}
              className="border-t border-slate-100 bg-blue-50/30"
            >
              <td className="p-2 text-slate-400 italic">원본 없음</td>
              <td className="p-2 text-right text-slate-400">-</td>
              <td className="p-2 text-blue-700 max-w-[160px] truncate">
                {ri.itemName}
              </td>
      ...
      >
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <span className="text-xs text-slate-400">
            {matched}/{bid.items.length}항목 매칭
          </span>
          <span className="text-xs text-slate-400">
...
function BidRequestCard({
  ...
  async function handleUpload(file: File) {
    ...
    try {
      ...
      const res = await fetch(`/api/bid-request/${req.id}/receive`, {
        method: "POST",
        body: fd,
      });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            비교 견적 요청 페이지에서 발송한 요청과 수령된 타사 견적을 원본과
            비교합니다.
          </p>
        </div>
        <Link
